...
export class NewOfferComponent {
	...
	selectChangeHandler(event: any) {
		...
		if (this.selectedCategory) {
			this.categoriesService.findCategoryAttributes(this.selectedCategory.id).subscribe(
				data => {
	...
	public save() {
		...
		if (category && this.activeUser && this.activeUser.id) {
			...
			this.productsService.insert(newProduct).subscribe({
				next: (result: Product) => {
					if (image1) this.imagesService.insert(new ImageRequest(null, image1, result)).subscribe();
					if (image2) this.imagesService.insert(new ImageRequest(null, image2, result)).pipe(delay(200)).subscribe();
					if (image3) this.imagesService.insert(new ImageRequest(null, image3, result)).pipe(delay(200)).subscribe();
					if (image4) this.imagesService.insert(new ImageRequest(null, image4, result)).pipe(delay(200)).subscribe();
					if (image5) this.imagesService.insert(new ImageRequest(null, image5, result)).pipe(delay(200)).subscribe();

					if (result.id) {
						this.productsService.findById(result.id).pipe(delay(1000)).subscribe({
							next: (result: Product) => {

								this.offersService.insert(new Offer(null, userId, result, true, false)).subscribe({
									next: (result: Offer) => {
										...
										for (let a of this.attributes) {
											...
											if (input && result.product.id) {
												this.productsService.insertValue(new Value(result.product.id, a.id, result.product.category.id, input)).pipe(delay(200)).subscribe();
											}
										}
